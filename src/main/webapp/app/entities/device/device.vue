<template>
  <div>
    <h2 id="page-heading" data-cy="DeviceHeading">
      <span v-text="$t('musicfilesApp.device.home.title')" id="device-heading">Devices</span>
      <div class="d-flex justify-content-end">
        ...
        >
          ...
          <span v-text="$t('musicfilesApp.device.home.createLabel')"> Create a new Device </span>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && devices && devices.length === 0">
      <span v-text="$t('musicfilesApp.device.home.notFound')">No devices found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="devices">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('musicfilesApp.device.name')">Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="device in devices" :key="device.id" data-cy="entityTable">
            ...
            <td class="text-right">
              <div class="btn-group">
                <router-link
                  :to="{ name: 'DeviceView', params: { deviceId: device.id } }"
                  tag="button"
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="musicfilesApp.device.delete.question" data-cy="deviceDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-device-heading" v-text="$t('musicfilesApp.device.delete.question', { id: removeId })">
